...
// `distributed actor`. Callers on the main thread invoke `send`/`report`; the arguments and
// the `MachineReport` return value cross the worker boundary as Codable. The machine itself
// (the non-Codable `MachineHost`) never leaves the worker.
distributed actor CounterWorker: WebWorker {
    ...
    // `importScripts`, which only exists in classic workers; the entry then dynamic-imports
    // the PackageToJS ES-module bundle (which `nil`/defaultScriptPath can't do — that
    // resolves to the `.wasm` path under WASI).
