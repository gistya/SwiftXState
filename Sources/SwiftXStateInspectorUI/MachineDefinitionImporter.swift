...
public extension InspectorStore {
    /// Parse a pasted XState machine definition and load it as a fresh actor, selecting it.
    /// Replaces any previously loaded definition (resets the store first).
    /// - Returns: the session id of the loaded actor.
    @discardableResult
    func loadDefinition(json: String, fallbackID: String = "pasted-machine") throws -> String {
